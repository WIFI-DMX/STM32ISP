		...
		case 1:	// read event 将定义的各种消息发送出去
			{
				...
				if (CommEvent & EV_RXFLAG) //接收到事件字符，并置于输入缓冲区中 
					ReceiveChar(port);//::SendMessage(port->m_pOwner, WM_COMM_RXFLAG_DETECTED, (WPARAM) 0, (LPARAM) port->m_nPortNr);
				if (CommEvent & EV_BREAK)  //输入中发生中断
					//::SendMessage(port->m_pOwner, WM_COMM_BREAK_DETECTED, (WPARAM) 0, (LPARAM) port->m_nPortNr);
//				{srdm.Received_state=0;b++;}
...
void CSerialPort::ReceiveChar(CSerialPort* port)
{
	...
	{ 
		...
		::SendMessage((port->m_pOwner), WM_COMM_RXCHAR, (WPARAM) port->buf, (LPARAM) comstat.cbInQue);
	} // end forever loop
...
bool CSerialPort::SetParity(BYTE btParity)
{
	if (m_hComm == NULL)
	{
		return false;
